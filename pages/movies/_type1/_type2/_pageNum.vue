  ...
  <section class="container">
    ...
    <div class="breadcrumbContainer">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>{{typeName}}电影</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="types">
    ...
    <ul class="move-list">
     <li v-for="(item, index) in moveList.contentList" :key="index">
       <a :href="'/detail/' + item.mainContent.id + '.html'" target="_blank">
         <img :alt="item.mainContent.title1 + '下载'" :src="item.mainContent.image4 ? 'http://static.biedese.cn/image2/' + item.mainContent.image4 : 'http://image.dfdy5.com/dianfeng/index/' + item.mainContent.image1 + '.jpg'">
    ...
    <div class="fenlei-pager">
      <el-pagination
        ...
        <my-pager :currentPage="+moveList.currPageNum" :pageCount="+moveList.totalPage" :pagerCount="7" :disabled="false" :path="'/movies/'+type1+'/'+type2+'/'"></my-pager>
      </el-pagination>
    </div>
    <script type="application/ld+json" v-if="moveList.contentList[0]">
     {
      "@context": "https://ziyuan.baidu.com/contexts/cambrian.jsonld",
      "@id": "http://www.dfdy5.com/movies/{{type1}}/{{type2}}/{{pageNum}}.html",
      "title": "{{this.TDK.T}}",
      "images": [
...
export default {
  ...
  head () {
    ...
    if (this.pageNum <= 1 && this.moveList.totalPage > 1) {
      arr.push({ rel: 'next', href: 'http://www.dfdy5.com/movies/'+this.type1+'/'+this.type2+'/'+(this.pageNum+1)+'.html' })
    } else if (this.pageNum > 1 && this.pageNum < this.moveList.totalPage) {
      arr.push({ rel: 'prev', href: 'http://www.dfdy5.com/movies/'+this.type1+'/'+this.type2+'/'+(this.pageNum-1)+'.html' })
      arr.push({ rel: 'next', href: 'http://www.dfdy5.com/movies/'+this.type1+'/'+this.type2+'/'+(this.pageNum+1)+'.html' })
  ...
  computed: {
    ...
    TDK () {
      ...
      if (typeName && type2Name) {
        return {
          D: `${type2Name}${typeName}电影下载、迅雷下载、在线观看、排行榜,${type2Name}${typeName}高清电影尽在巅峰电影网，${type2Name}${typeName}电影资源大全网站。`,
          K: `${type2Name}${typeName}电影下载，${type2Name}${typeName}电影迅雷下载，${type2Name}${typeName}电影在线观看，${type2Name}${typeName}电影排行榜,${type2Name}高清${typeName}电影资源`,
          T: `${type2Name}高清${typeName}电影资源大全_迅雷下载_在线观看_第${this.pageNum}页_巅峰电影网`
        }
      } else if (typeName && !type2Name) {
        return {
          D: `${typeName}电影下载、迅雷下载、在线观看、排行榜,${typeName}高清电影尽在巅峰电影网，${typeName}电影资源大全网站。`,
          K: `${typeName}电影下载，${typeName}电影迅雷下载，${typeName}电影在线观看，${typeName}电影排行榜,高清${typeName}电影资源`,
          T: `高清${typeName}电影资源大全_迅雷下载_在线观看_第${this.pageNum}页_巅峰电影网`
        }
      } else if (!typeName && type2Name) {
        return {
          D: `${type2Name}电影下载、迅雷下载、在线观看、排行榜,${type2Name}高清电影尽在巅峰电影网，${type2Name}电影资源大全网站。`,
          K: `${type2Name}电影下载，${type2Name}电影迅雷下载，${type2Name}电影在线观看，${type2Name}电影排行榜,${type2Name}高清电影资源`,
          T: `${type2Name}高清电影资源大全_迅雷下载_在线观看_第${this.pageNum}页_巅峰电影网`
        }
      } else if (!typeName && !type2Name) {
        return {
          D: `最新电影下载、迅雷下载、在线观看、排行榜,高清大全尽在巅峰电影网，全网最多的电影资源网站。`,
          K: `最新电影下载，迅雷下载，在线观看，电影排行榜,电影大全`,
          T: `最新高清电影大全_迅雷下载_在线观看_巅峰电影网`
        }
      }
    }
  },
  async asyncData (_this) {
    ...
    getContentListByTypeMessage.productCode = 'dianyingzhan'
    ...
    let x = await getType1List(getTypeListMessage)
    let x2 = await getType2List(getTypeListMessage)
    let x3 = await getContentListByType(getContentListByTypeMessage)
  ...
  methods: {
    ...
    changePage (page) {
      ...
      this.jump = true
      location.href = `/movies/${this.type1}/${this.type2}/${page}.html`
    }
  },
  mounted () {
